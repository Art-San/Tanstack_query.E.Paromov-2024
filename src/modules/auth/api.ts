...
export const authApi = {
	...
	loginUser: ({ login, password }: { login: string; password: string }) => {
		// Тут не стали запрос оборачивать queryOptions, так как не все запросы стоит кешировать
	...
	// 	// Тут не стали запрос оборачивать queryOptions, так как не все запросы стоит кешировать
	// 	try {
	// 		const res = await jsonApiInstance<UserDto[]>(
	// 			`/users?login=${login}&password=${password}`
	// 		)
	// 		return res[0] as UserDto | undefined
	// 	} catch (error) {
	// 		console.log(error)
	// 	}
	// },
